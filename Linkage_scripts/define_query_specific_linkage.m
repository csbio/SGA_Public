function lnkg = define_query_specific_linkage(sga)


data = struct();

data.queries = sga.Cannon.Orf(sga.Cannon.isQuery);
data.arrays = sga.Cannon.Orf(sga.Cannon.isArray);
data.scores_eps = sga.eps(sga.Cannon.isQuery, sga.Cannon.isArray);

lnkg = AB_define_query_specific_linkage_110418(data);
...
function lnkg = AB_define_query_specific_linkage_110418(dataset)
